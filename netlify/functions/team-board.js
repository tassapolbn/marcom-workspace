...
const STYLE = '<style>'
  ...
  // hero
  + '.hero{position:relative;overflow:hidden;background:linear-gradient(150deg,#1a3d66 0%,#0b2547 55%,#0a2040 100%);color:#fff;border-radius:22px;padding:26px 26px 22px;box-shadow:0 24px 50px -24px rgba(12,26,52,.7);animation:heroIn .7s cubic-bezier(.2,.7,.2,1) both;}'
  + '.hero:before{content:"";position:absolute;inset:-45% -12% auto auto;width:560px;height:560px;background:radial-gradient(circle at 30% 30%,rgba(120,170,255,.18),rgba(120,170,255,0) 68%);filter:blur(10px);animation:float 12s ease-in-out infinite;}'
  + '.hero:after{content:"";position:absolute;inset:auto auto -65% -14%;width:520px;height:520px;background:radial-gradient(circle at 50% 50%,rgba(240,179,35,.14),rgba(240,179,35,0) 68%);filter:blur(8px);animation:float 15s ease-in-out infinite reverse;}'
